...
// clear images max_srcset_image_width
add_filter( 'max_srcset_image_width', create_function('', 'return 1;'));

// set blog excerpt_length
...
// wp_enqueue_style, wp_enqueue_script  这两个方法会插入到 body 的最后面, 本地手机调试无法加载资源
function loadTimeResource($srcList){
	$noLoad = array();
	foreach ($srcList as $key => $src) {
		if(preg_match('/\.css$/',$src) == 1){
			// wp_enqueue_style($key, $src, [], filemtime($_SERVER['DOCUMENT_ROOT'] . $src));
			echo '<link href="'.$src.'?ver='.filemtime($_SERVER['DOCUMENT_ROOT'] . $src).'" rel="stylesheet">';
		} else if(preg_match('/\.js$/',$src) == 1){
			// wp_enqueue_script($key, $src, [], filemtime($_SERVER['DOCUMENT_ROOT'] . $src));
			echo '<script src="'.$src.'?ver='.filemtime($_SERVER['DOCUMENT_ROOT'] . $src).'"></script>';
		} else {
			array_push($noLoad,$key);
		}
	}
...
function loadTimeStylesheet($srcList){
	foreach ($srcList as $key => $src) {
		wp_enqueue_style($key, $src,
		[], // 依赖
		filemtime($_SERVER['DOCUMENT_ROOT'] . $src) // 版本
		);
	}
}

function loadTimeScript($srcList){
	foreach ($srcList as $key => $src) {
		wp_enqueue_script($key, $src,
		[], // 依赖
		filemtime($_SERVER['DOCUMENT_ROOT'] . $src) // 版本
		);
	}
...
add_image_size( 'blog_img', 976, 548, true );
